//停用或启用课件
function enableORstopCourseWare(id,state){
    layer.confirm('是否启用？', {icon: 3, title:'提示'}, function(index){
        //执行启用
        $.ajax({
            url : contextPath + '/manage/courseController/enableORstopCourseWare',
            timeout : 300000,
            dataType : "json",
            type : "post",
            data : {
...
function addCourseWare(id){
    layer.open({
        ...
        area: ['100%', '100%'],  //大小
        content: contextPath + '/manage/courseController/courseWare_add?course_id='+id
    });
}
//打开修改视频页面
